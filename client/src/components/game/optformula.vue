<template>
    <div class="fomula" :id="idNum">
        <form>
            <div class="row">
                <div class="col-2">
                    <label :for="'dev'+idNum">Game Type</label>
                    <select :id="'dev'+idNum" class="form-control" v-model="selectedC">
...
<script>
export default {
    props:["idNum","activities","device1","device2","device3","device4"],
    data(){
        return {
            selected:'',
            lwh:'',
    ...
    watch:{
        selectedA () {
          const targetDevice = this.device.find((item) => (item.devicename == this.selectedA));
            ...
            if (targetDevice.devicename=='Microsoft Xbox'||targetDevice.devicename=='Sony PlayStation'||targetDevice.devicename=='Nintendo Switch') {
              this.result.traffic = parseInt(0);
            }
        },
        selectedB () {
        ...
        selectedC () {
          ...
          this.result.activity = this.selectedC;
        },
        picked(){
            this.result.wh = parseFloat(this.picked);
        },
        time(){
            let reg = /^\+?[0-9][0-9]*$/;
