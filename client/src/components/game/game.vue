<template>
  <div id="game" class="d-flex flex-column my-5">
    <div class="mx-auto col-10 py-1 my-2 row" id="intro">
      <h2 class="mx-auto col-3">Gaming Preference</h2>
      <BaseNav class="col-9"></BaseNav>
    </div>
    <div class="mx-auto alert alert-info col-8" role="alert">
      Games are divided into console games, PC games, and mobile games based on the device carrier.
    </div>
    ...
    <div id="inputInfo" class="mx-auto p-3">
      <div id="fcontent">
        <Fixformula v-for="item in gameType" :key="item.id" :idNum="item.id" :activity="item.activity" :traffic="item.traffic" :device1="device1" :device2="device2" :device3="device3" :device4="device4" @getSingleResult="setSingleResult"></Fixformula>
      </div>
      <div id="ocontent" v-if="finalResult.length > 3">
        <Formula v-for="item in clearFinalResult" :key="item.id" :idNum="item.id" :activities="gameType" :device1="device1" :device2="device2" :device3="device3" :device4="device4" @delFormula="delFormula" @getSingleResult="setSingleResult"></Formula>
      </div>
...
<script>
import BaseNav from '../common/nav.vue'
import Fixformula from '../game/gameformula.vue' 
import Formula from '../game/optformula.vue'
...
export default {
  ...
  data(){
    return{
      ...
      gameType:[
        {id:'1',activity:"Console Game",traffic:0},
        {id:'2',activity:"PC Game",traffic:0},
        {id:'3',activity:"Mobile Game",traffic:0},
      ],
      device1:[
        {id:'1',devicename:"Microsoft Xbox"},
        {id:'2',devicename:"Sony PlayStation"},
        {id:'3',devicename:"Nintendo Switch"},
      ],
      device2:[
        {id:'4',devicename:"Smartphone"},
        {id:'5',devicename:"Tablet"},
      ],
      device3:[
        {id:'6',devicename:"Laptop PC"},
        {id:'7',devicename:"Desktop PC"},
      ],
      device4:[
        {id:'8',devicename:"TV"},
        {id:'9',devicename:"Projector"},
      ],
      fixedNum:0,
      finalResult:[],
      nextBtn:{
  ...
  methods:{
    initFinalResult(){
      this.fixedNum = this.gameType.length;
      for (let i = 0; i < this.fixedNum; i++) {
        this.finalResult.push({
          "id":i+1,
          "activity":this.gameType[i].activity,
          "time":null,
          "gameName":'null',
          "traffic":null,
          "device":null,
    ...
    setSingleResult(singleRes){
      ...
      this.finalResult.forEach(item => {
        if (item.id === selectId){
          ...
          this.$set(item, 'gameName',singleRes.gameName);
